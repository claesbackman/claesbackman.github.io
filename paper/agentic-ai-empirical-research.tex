...
rather than merely busy, which file formats need converting, and how to record
project context so it does not have to be re-explained every session. The
second half, which is the part I think matters more, is verification: a
...
verification happens while the code is being written rather than afterwards.
...
where the tool-specific material is kept current. Sections \ref{sec:surfaces}
and Appendix \ref{app:tools} describe software that changes every few months and
should be read against the dated web version. The argument in
Sections \ref{sec:loop}--\ref{sec:habits} is meant to outlast the products it
was written about.
...
\item \textbf{People who have not started.} Sections \ref{sec:surfaces} and
  ...
  first hour so that the tool is actually useful on a research project rather
  than a toy.
...
\begin{aside}
\textbf{Two cautions before anything else.} First, never paste data subject to a
confidentiality agreement --- administrative registers, restricted samples,
anything under a data-use agreement --- into a cloud agent or a web chat.
Local-only surfaces are the right tool there, and Section \ref{sec:confidential}
returns to this. Second, the model's confidence is not calibrated. Verify
regression results, citations, and identification claims yourself before they
enter a draft. The rest of this document is largely an elaboration of that
second sentence.
\end{aside}
...
\emph{surface} --- where the agent runs. There are six, and most researchers end
...
but both now include modes aimed at sustained work rather than conversation:
...
sustained editing, use a code-oriented surface or an IDE --- chat-only surfaces
become hard to track once many files and diffs are involved.
...
preference, VS Code remains the path of least resistance --- it is what most
documentation, screenshots, and shared workflows assume.
...
directory tree to be useful --- to see that \code{01\_clean.R} produces the file
...
commands, inspect variables, and read stored results rather than writing
do-files blind.
...
the tables''; ``download monthly CPI from FRED and save as CSV'') and the agent
...
characters-on-a-page rather than a rendered binary, the better the agent can
...
\addlinespace
\code{.xlsx .xls} &
Convert to CSV first; the binaries cannot be read directly. Split multi-sheet
...
when you want a figure improved. Text cannot be reliably extracted from scans;
use OCR first. \\
\addlinespace
\code{.qsf} (Qualtrics) &
...
\item \textbf{Long PDFs} (50+ pages) should be converted to markdown first.
  ...
  to navigate.
\item \textbf{Scanned PDFs} cannot be read as text at all --- they are images of
  pages. Run OCR first (\code{ocrmypdf} is a good free option), then convert.
...
documents rather than inlining them --- see the note on \code{decisions.md} at the
end of Section \ref{sec:loop}.
\end{aside}

\subsection{Skills: reusable workflows}

Both systems let you save a set of instructions as a named, reusable workflow ---
a markdown file that you invoke by name to run a procedure you repeat across
...
\emph{Point at specific files} rather than letting the agent read everything.
...
the checks in the next section become confirmation rather than discovery.
...
have an explicit mode for this; otherwise say so in the prompt.
...
something the agent has to run rather than assert.
...
one-line pointer to it from your project-context file rather than the decisions
...
Run it in a new session rather than the one that made the change. Starting fresh
...
what the paper says it does --- the failure mode no amount of reading the do-file
will surface, because the do-file is internally consistent. Use a different model
...
instruction to stop at ambiguities is also useful in its own right --- the
ambiguities it surfaces are ones you may want to resolve in the paper, not only
in the check.
...
Most of the above is agnostic about where the work happens. This part is not.

If your data is subject to a confidentiality agreement --- administrative
registers, restricted samples, anything under a data-use agreement --- it does not
go into a cloud agent or a web chat. That rules out the web surface entirely,
rules out cloud delegation from the IDE, and means that for those projects you
work locally, or on the secure environment itself via the command line.

This has consequences for the verification ladder that are worth stating plainly,
because they are easy to miss. Check 2 requires two vendors, which means two sets
of terms to check rather than one. Check 4 involves creating a second folder with
a description of the data in it: that description is a derived product of the
restricted data, and whether it can leave the environment is a question for your
data-use agreement, not for the agent. In secure environments the realistic
version of Check 4 is a second implementation run inside the environment, by an
agent running there, on the terminal surface.

None of this makes the checks impossible. It does mean the setup work happens
once, deliberately, before the first session rather than in the middle of one.

% =============================================================== SECTION 8 ==
\section{Where this is going}

Three things seem worth saying about the direction of travel, offered as
expectations rather than predictions.
...
becomes routine rather than heroic. The habits in Section \ref{sec:habits} and the
...
about disclosure of use but about documentation of checking --- closer to what we
already expect for a replication package than to a conflict-of-interest
statement. The \code{decisions.md} file in Section \ref{sec:loop} is a small bet
on that.
...
it then becomes \code{/quiz-me}. Run it in a new session, not the one that made
the change.
...
for. For a verification skill this is usually not what you want --- the point is
...
\endhead
Project\newline instructions &
\code{AGENTS.md}. Reads global and project files before working; layers root and
nested instructions, closer files taking precedence. &
...
\addlinespace
Reasoning &
Explicit reasoning-effort control in the IDE. Start at medium; raise for complex
debugging or ambiguous empirical logic. &
Model selection in the panel; plan review and checkpoints in the interface. \\
\bottomrule
\end{longtable}
...
That is not a hedge --- it is the check.
...
\item \textbf{The first sync overwrites.} Make sure both sides agree on initial
  contents before linking. If one side is empty, the other wins --- and not always
...
\item \textbf{Dropbox conflicts are silent.} Simultaneous edits produce a
  \code{(conflicted copy).tex} file rather than a warning. Skim the folder
  occasionally.
...
experience and from others. In particular: Geoffrey Litt on understanding as the
new bottleneck, and the \emph{explain-diff} skill that Section \ref{sec:quiz} and
...
Section \ref{sec:diff} comes from, and on writing with AI assistance; and Scott
Cunningham, who is the person I associate with the cross-language reimplementation
check in Section \ref{sec:reimplement}.

For the broader picture of what these tools can do in economics, rather than how
to check them, Anton Korinek's survey in the \emph{Journal of Economic Literature}
