...
The more consequential early decision is which \emph{surface} you use, meaning
where the agent runs. There are six, and most researchers end
...
commands, inspect variables, and read stored results. It is not writing do-files
blind.

For \textbf{Python}, install the core extension, a language server, the debugger,
and environment management. Do this \emph{even if you do not write Python}, which is the part people skip. A surprising share of the everyday
...
\textbf{For verification, it stops being optional}, and it is the setup step I
would argue hardest for. The reason is
...
The practical advice is all downstream of that.

Cost grows with session length even when the work does not.
...
Section \ref{sec:context-window} is therefore also cost discipline.

A stable prefix is also cheap. Most vendors cache repeated context,
...
Spend on judgement, economise on typing.
...
Ask the current agent to launch a subagent to review the change, or start a new
session yourself pointed at the diff.
...
pipeline and the numbers match, you get the check for free. Some of the
verification happens without you planning it.
...
This is the most important practical point in this document, and it generalises
well beyond this check: every claim of independence in
...
Running it yourself matters, and not only for the token cost. An agent that runs its own
script and sees an implausible number will debug towards plausibility, and
plausibility is exactly what you are trying to test. The
...
Two exceptions are real. Models running entirely on your own hardware keep
...
hosted frontier models. That tradeoff is real. Weigh it before dismissing it.
...
a judgement your data-use agreement governs, so ask the data provider instead of
guessing.
...
The easy rule to follow is the one about not uploading the microdata. Everything
downstream of it is easier to break.
...
Three expectations about the direction of travel, offered as expectations and
not predictions.
...
do with the answer. My own guess is that the norm that eventually forms is
